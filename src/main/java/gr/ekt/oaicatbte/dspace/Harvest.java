...
public class Harvest
{
	...
	public static List harvest(Context context, ArrayList<DSpaceObject> scopes,
		...
		if (hasCommunity)
			queryCom = "(SELECT DISTINCT handle.handle, handle.resource_id, item.withdrawn, item.last_modified FROM handle, item";
		if (hasCollection)
		...
		if (communityScopes.size() > 0)
			queryCom += ", communities2item";
		if (collectionScopes.size() > 0)
			queryCol += ", collection2item";

		if (hasCommunity)
			queryCom += " WHERE handle.resource_type_id=" + Constants.ITEM + " AND handle.resource_id=item.item_id ";
		if (hasCollection)
		...
		if (communityScopes.size() > 0){
			queryCom += " AND (";
			int counter = 0;
			for (DSpaceObject scope : communityScopes){
				if (counter == 0)
					queryCom += "communities2item.community_id= ?";
				else
					queryCom += " OR communities2item.community_id= ?";
				counter++;
				parametersCom.add(new Integer(scope.getID()));
			}
			queryCom += ") AND communities2item.item_id=handle.resource_id ";
		}

		if (collectionScopes.size() > 0){
			queryCol += " AND (";
			int counter = 0;
			for (DSpaceObject scope : collectionScopes){
				if (counter == 0)
					queryCol += "collection2item.collection_id= ?";
				else
					queryCol += " OR collection2item.collection_id= ?";
				counter++;
				parametersCol.add(new Integer(scope.getID()));
			}
			queryCol += ") AND collection2item.item_id=handle.resource_id ";
		}      

		if (startDate != null)
		{
			if (hasCommunity){
				queryCom = queryCom + " AND item.last_modified >= ? ";
				parametersCom.add(toTimestamp(startDate, false));
		...
		if (endDate != null)
		{
			...
			if (hasCommunity){
				queryCom += " AND item.last_modified <= ? ";
				parametersCom.add(toTimestamp(endDate, selfGenerated));
			}
			if (hasCollection){
		...
		//Now we have to make the general query...
		String query = "SELECT * FROM ( ";

		if (hasCommunity)
			query += queryCom;

		if (hasCommunity && hasCollection){
			query += " UNION " + queryCol;
		}
		else {
			query += queryCol;
		}

		query += ") AS A ORDER BY A.resource_id";

		List parameters = new ArrayList();
		for (Object s : parametersCom){
			parameters.add(s);
		}
